...
//		dd($goals);
    ...
    left join (select cards.player_id, count(player_id) as y_cnt from cards where deleted_at is null and color='yellow' and is_cleared=0 and league_id = " . $id . " group by player_id) as y_card on y_card.player_id=p.id 
    left join (select cards.player_id, count(player_id) as r_cnt from cards where deleted_at is null and color='red' and is_cleared=0 and league_id = " . $id . " group by player_id) as r_card on r_card.player_id=p.id 
    where (y_card.y_cnt != 0 or r_card.r_cnt != 0) " . $whereCond . " order by " . $orderby);

		return view('team.league.warning')->with(compact('players', 'league', 'opval', 'teams'));
	}

	public function wmatch($id, $player_id)
	{
		$league = Leagues::find($id);
		$player = Players::find($player_id);
		$cards = Cards::where('deleted_at', null)->where('is_cleared', 0)->where('player_id', $player_id)->get();
