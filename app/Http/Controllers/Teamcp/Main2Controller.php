...
class Main2Controller extends Controller
{
	...
	public function top(Request $req)
	{
		...
		$matches = Matches::with('leagueOne', 'home0', 'away0', 'place')->where('is_filled', 0)->where('match_date', '<=', date('Y-m-d', strtotime("+3 days")))->where(function ($q) {
			$q->where('home_id', \Session::get('team_id'))->orWhere('away_id', \Session::get('team_id'));
		});
		...
		$matches0 = Matches::with('leagueOne', 'home0', 'away0', 'place')->where('is_filled', 1)->where('match_date', '<=', date('Y-m-d', strtotime("+3 days")))->where(function ($q) {
			$q->where('home_id', \Session::get('team_id'))->orWhere('away_id', \Session::get('team_id'));
		});
		$matches0 = $matches0->orderBy('match_at', 'desc')->get();

		$team = Teams::leftJoin('team_yearly_group', 'team_yearly_group.team_id', '=', 'teams.id')->where('yyyy', config('app.nendo_backend'))->find(\Session::get('team_id'));
 ...
 left join (select goals.goal_player_id, count(goal_player_id) as g_cnt from goals where deleted_at is null and created_at >= '2021-03-01' and team_id=" . $match->home_id . " group by goal_player_id) as g on g.goal_player_id=p.id
 left join (select cards.player_id, count(player_id) as y_cnt from cards where deleted_at is null and created_at >= '2021-03-01' and team_id=" . $match->home_id . " and color='yellow' and is_cleared=0 group by player_id) as y_card on y_card.player_id=p.id 
 left join (select cards.player_id, count(player_id) as r_cnt from cards where deleted_at is null and created_at >= '2021-03-01' and team_id=" . $match->home_id . " and color='red' and is_cleared=0 group by player_id) as r_card on r_card.player_id=p.id 
 ...
 left join (select goals.goal_player_id, count(goal_player_id) as g_cnt from goals where deleted_at is null and created_at >= '2021-03-01' and team_id=" . $match->away_id . " group by goal_player_id) as g on g.goal_player_id=p.id
 left join (select cards.player_id, count(player_id) as y_cnt from cards where deleted_at is null and created_at >= '2021-03-01' and team_id=" . $match->away_id . " and color='yellow' and is_cleared=0 group by player_id) as y_card on y_card.player_id=p.id 
 left join (select cards.player_id, count(player_id) as r_cnt from cards where deleted_at is null and created_at >= '2021-03-01' and team_id=" . $match->away_id . " and color='red' and is_cleared=0 group by player_id) as r_card on r_card.player_id=p.id 
  where p.deleted_at is null and p.team_id=" . $match->away_id);
	...
	{
		$team = Teams::leftJoin('team_yearly_group', 'team_yearly_group.team_id', '=', 'teams.id')->where('yyyy', config('app.nendo_backend'))->findOrFail(session('team_id'));
	...
	public function update(Request $req)
	{
		...
		$input = $req->except('id', '_token', 'emblem_img', 'group_img', 'zip');
		// $team = (object)$req->except('_token', 'files', 'zip');
