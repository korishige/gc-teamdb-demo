...
class MatchController extends Controller
{
	...
	{
		// dd(\Input::all());
		$input_match = $req->only('home_pt', 'away_pt', 'note', 'home_comment', 'away_comment');
		...
		if ($match->home_id == $req->session()->get('team_id')) {
			$input_match = $req->only('home_pt', 'away_pt', 'note', 'home_comment');
			...
			foreach (array_unique($_players) as $p) {
				...
				$y_count = Cards::where('player_id', $p)->where('color', 'yellow')->where('is_cleared', 0)->count();
				$r_count = Cards::where('player_id', $p)->where('color', 'red')->where('is_cleared', 0)->count();
